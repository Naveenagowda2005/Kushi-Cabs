...
-- The vendor_document_type enum already exists, so we skip that
-- This only creates the missing tables

-- Skip the CREATE TYPE since it already exists
...
CREATE TABLE IF NOT EXISTS vendor_verification_status (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE INDEX IF NOT EXISTS idx_vendor_documents_user_id ON vendor_documents(user_id);
CREATE INDEX IF NOT EXISTS idx_vendor_verification_status_vendor_id ON vendor_verification_status(vendor_id);
CREATE INDEX IF NOT EXISTS idx_vendor_verification_status_user_id ON vendor_verification_status(user_id);
CREATE INDEX IF NOT EXISTS idx_vendor_verification_status_overall_status ON vendor_verification_status(overall_status);
...
DROP TRIGGER IF EXISTS trg_sync_vendor_verification_status ON vendor_verification_status;
