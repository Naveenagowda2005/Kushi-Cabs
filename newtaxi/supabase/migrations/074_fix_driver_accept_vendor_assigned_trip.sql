-- ============================================================
-- FIX: Driver Accept Vendor-Assigned Trips (Updated)
-- Migration: 074_fix_driver_accept_vendor_assigned_trip.sql
-- Purpose: Allow drivers to accept vendor-assigned trips (status = accepted, driver_id set)
-- NOTE: Keep status = accepted after driver accepts, only change to in_progress when trip actually starts
-- ============================================================

-- Update accept_trip function to handle vendor-assigned trips
-- Key change: For drivers, check if trip is vendor-assigned (driver_id IS NOT NULL)
--   - If vendor-assigned: allow status 'pending' OR 'accepted'
--   - If public: only allow status 'pending'
-- STATUS: Keep as 'accepted' (driver clicks "Start Trip" to change to 'in_progress')
...
-- ============================================================
-- Trip Status Flow:
-- 1. Vendor accepts trip → status = 'accepted'
-- 2. Vendor assigns to driver → driver_id set, status stays 'accepted'
-- 3. Driver accepts trip → status stays 'accepted' (no change)
-- 4. Driver clicks "Start Trip" + uploads odometer → status changes to 'in_progress'
-- 5. Driver completes trip → status changes to 'completed'
-- ============================================================
